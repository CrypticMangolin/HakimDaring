...
import ModeBahasa from '../core/Data/ModeBahasa'
...
function HalamanPengerjaan() {
  ...
  let [daftarModeBahasa, ] = useState<ModeBahasa[]>(daftarBahasa.ambilBahasa())
    ...
    <Container className='min-vh-100 min-vw-100 m-0 p-0 d-flex flex-column'>
      ...
      <Row className='m-0 p-0'>
        <Col sm={12} md={12} lg={6} xl={6} className="d-flex flex-column m-0 p-2">
          <Row className='m-0 mx-4 p-0 d-flex flex-column'>
            ...
            <Row className='m-0 mb-2 p-0 d-flex flex-row'>
              <Col xs={2} className='m-0 p-0'>
                <Form.Select size='sm' value={daftarModeBahasa.length > 0 ? bahasaDipilih : ""} onChange={pilihBahasa}>
                  {daftarModeBahasa.map((value : ModeBahasa, index : number) => (
              ...
              {inputCustom.map((value : string, index : number) => (
                <Row key={index} className='m-0 p-0 d-flex flex-row mb-1' xs={12}>
                  ...
                  <Col xs={5} className='m-0 p-0'>
                    <textarea className='m-0 w-100' rows={3} style={{"resize" : "none"}} onChange={(e) => {gantiCostumInput(index, e.target.value)}} value={value}/>
                  </Col>
                  <Col xs={5} className='m-0 p-0'>
                    <textarea className='m-0 w-100' rows={3} style={{"resize" : "none"}}/>
                  </Col>
                </Row>
              ))}
              <Row className='m-0 p-0 d-flex flex-row'>
                ...
                <Col className='m-0 mb-1 p-0' xs={4}>
                  <Button variant='light' className='btn-block m-0 rounded-0 border border-dark' onClick={() => {}}>Run Testcase</Button>
                </Col>
                <Col className='m-0 mb-1 p-0' xs={4}>
